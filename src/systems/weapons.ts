...
export class WeaponSystem {
  ...
  tryPickup(player: Player): void {
    ...
    this.heldImages[player.idx] = this.scene.add
      ...
      .setScale(isPngKey(`wpn_${id}`) ? weaponDisplayScale(this.scene.textures.get(`wpn_${id}`).getSourceImage().height) : 2)
      .setDepth(11);
      ...
      if (pl.idx === pr.ownerIdx) {
        if (
          ...
          this.heldImages[pl.idx] = this.scene.add
            ...
            .setScale(isPngKey(`wpn_${pr.def.id}`) ? weaponDisplayScale(this.scene.textures.get(`wpn_${pr.def.id}`).getSourceImage().height) : 2)
            .setDepth(11);
          this.removeProjectileAt(i);
          break;
        }

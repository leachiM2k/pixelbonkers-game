...
import { SHEET_SCALE, weaponDisplayScale } from './sheetScale';
...
export class SpritePreviewScene extends Phaser.Scene {
  ...
  private section(
    ...
    sprites.forEach((s, i) => {
      ...
      this.add.image(x, bottom, s.key).setOrigin(0.5, 1).setScale(isPngKey(s.key) ? (s.key.startsWith('wpn_') ? weaponDisplayScale(s.rows.length) : SHEET_SCALE) : scale);
  ...
  private place(key: string, x: number, bottom: number, scale: number): void {
    ...
    this.add.image(x, bottom, key).setOrigin(0.5, 1).setScale(isPngKey(key) ? (key.startsWith('wpn_') ? weaponDisplayScale(this.textures.get(key).getSourceImage().height) : SHEET_SCALE) : scale);
